...
description: API reference for OpenClaw SimpleX gateway RPC methods used for runtime status, invite links, contact requests, group links, and operator-controlled SimpleX onboarding.
---

The plugin registers these gateway RPC methods for runtime diagnostics, invite/address automation, contact-request review, group-link lifecycle, and operator-controlled SimpleX link onboarding.

<Info>
  For day-to-day invite/address workflows, prefer the plugin CLI commands in [Example Commands](/reference/example-commands). Use these gateway methods when another system needs programmatic access to runtime status, contact-request review, group-link lifecycle, or link onboarding.
</Info>
...
- The SimpleX Node runtime must start successfully for that account.
- If <code>accountId</code> is omitted or blank, the plugin uses the channel's default account.
- These methods call the embedded SimpleX runtime API for the target account directly; they do not start a separate SimpleX CLI or WebSocket sidecar.
...
- `address` -> returns or creates the account address link
...
List the current address link and related contact state through the SimpleX runtime APIs.
...
Revoke the active address link through the SimpleX runtime API.
...
- `packageVersion`: installed `simplex-chat` package version
...
```json
{
  ...
  "configured": true,
  "mode": "node",
  "dbFilePrefix": "~/.simplex/simplex_v1",
...
Create SimpleX groups and manage group invite links through the official Node runtime.
