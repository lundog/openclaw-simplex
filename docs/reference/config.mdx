...
| `connection.outboundFolder` | Directory OpenClaw stages outgoing files into so the external runtime can read them on send. Only needed when the runtime doesn't share a filesystem with OpenClaw (e.g. the runtime runs in a container): both mount the same directory at the same path, and OpenClaw passes that path in the send command. Unset = the local file path is passed to the runtime as-is (single-filesystem default). |
