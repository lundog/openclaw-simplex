...
The config surface is intentionally small: local database prefix, initial profile labels, migration behavior, file auto-accept, timeout, and OpenClaw policy/UX fields. Legacy CLI/WebSocket fields such as <code>wsUrl</code>, <code>host</code>, <code>port</code>, <code>token</code>, <code>managed</code>, and <code>cliPath</code> are not supported and are removed by <code>openclaw simplex migrate</code>.
