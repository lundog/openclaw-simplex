...
| `dbFilePrefix` | SQLite database file prefix for the Node runtime. Required; the plugin does not force a default. |
...
<Note>
  A channel/account is considered configured when <code>dbFilePrefix</code> is set. The official SimpleX Node API requires this database prefix.
</Note>
