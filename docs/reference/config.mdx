...
| `connection.allowUnsafeRemoteWs` | Explicitly allow plaintext `ws://` to a non-loopback host. Use only for private sidecar networks or firewall-protected deployments. |
...
- <code>connection.allowUnsafeRemoteWs</code> when a private sidecar network requires a non-loopback host such as <code>simplex-chat</code>
