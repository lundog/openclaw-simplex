...
description: Copy-paste CLI commands for common OpenClaw SimpleX tasks like setup, runtime diagnostics, one-time invites, SimpleX addresses, contact requests, group links, pairing approval, and migration.
---
...
## One-time invites and SimpleX addresses
...
List current SimpleX address state:

```bash
openclaw simplex invite list
```

Show the current SimpleX address:

```bash
openclaw simplex address show --qr
```

Create or return the current SimpleX address:

```bash
openclaw simplex address create --qr
```

Revoke the current SimpleX address:
...
Migrate older <code>simplex</code> ids and legacy WebSocket/CLI runtime config to the current <code>openclaw-simplex</code> Node runtime shape:
