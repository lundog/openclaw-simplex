...
You can also generate or inspect links directly in the <code>simplex-chat</code> console with <code>/c</code>, <code>/ad</code>, or <code>/show_address</code>. For automation, OpenClaw also exposes gateway methods for invites, runtime status, contact requests, group links, and link onboarding.
