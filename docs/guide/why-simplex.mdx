...
This plugin takes a different shape. With SimpleX, the agent becomes reachable when you hand someone a one-time invite link or share the account's reusable SimpleX address. There is no phone-number binding and no hosted bot account to provision before the first conversation starts.
...
<Columns cols={2}>
  <Card title="Invitation-first reachability" icon="key-round">
    Create a one-time connect link for bounded onboarding, or share a reusable SimpleX address when you want ongoing reachability.
  </Card>
...
<Columns cols={2}>
  <Card title="Operator-controlled runtime" icon="server">
    Use the official SimpleX Node runtime and choose where the local database and relay path live.
  </Card>
...
A SimpleX channel backed by the official Node runtime:
...
- gateway methods for runtime diagnostics, contact-request review, group-link lifecycle, and operator link onboarding
...
Without this plugin, using SimpleX with OpenClaw usually means custom integration work around event parsing, outbound runtime API calls, pairing state, and invite management. This package turns that into a standard channel implementation.
...
- access starts from a one-time invite link or SimpleX address, not a public platform bot identity
