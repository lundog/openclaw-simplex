...
This is the mirror image of inbound, with one crucial difference. Inbound is name-only, so the two sides may mount the shared directory at <em>different</em> paths. Outbound sends a full path <em>to</em> the runtime, so both sides must see it at the <strong>same</strong> path. Mount a shared volume verbatim — the identical absolute path inside the runtime and wherever OpenClaw runs — and point <code>connection.outboundFolder</code> at it:
...
OpenClaw stages `pic.jpg` into `/tmp/simplex-outbound` and passes `/tmp/simplex-outbound/pic.jpg`; the runtime reads that same path, then the staged file is removed after the send. (If OpenClaw runs on the host rather than in a container, bind-mount a host directory onto the identical path inside the runtime container.)
