...
<Tip>
  Two common mistakes: the channel section is missing, or <code>dmPolicy: "pairing"</code> is enabled but the sender has not been approved yet.
</Tip>
...
What to check first: <code>channels.openclaw-simplex</code> must exist. The simplest valid config is:
...
- the SimpleX default database path is writable, or <code>dbFilePrefix</code> points to a writable prefix
...
Run the runtime doctor from the same OpenClaw environment:
...
- run <code>openclaw simplex runtime status</code> to verify the active account and last SimpleX error
