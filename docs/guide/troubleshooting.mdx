...
<Tip>
  Two common mistakes: <code>channels.openclaw-simplex.dbFilePrefix</code> is missing, or <code>dmPolicy: "pairing"</code> is enabled but the sender has not been approved yet.
</Tip>
...
What to check first: <code>channels.openclaw-simplex.dbFilePrefix</code> must exist. The simplest valid config is:
