...
<Steps>
  ...
  <Step title="Configure the channel">
    ...
    <Info>
      By default, the plugin uses the same database prefix as the SimpleX terminal CLI: <code>~/.simplex/simplex_v1</code> on Linux/macOS and <code>%APPDATA%/simplex/simplex_v1</code> on Windows. Set <code>dbFilePrefix</code> only when you want a separate bot identity or a named account.
    </Info>
  </Step>
