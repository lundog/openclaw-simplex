---
title: How It Works
description: Trace the full message lifecycle from an incoming SimpleX contact through the plugin runtime bridge, OpenClaw policy checks, and agent response delivery.
---

The runtime model is intentionally narrow: OpenClaw owns routing and policy, this plugin translates channel behavior, and SimpleX handles encrypted transport through the official Node runtime.
...
<Steps>
  ...
  <Step title="The plugin attaches to your SimpleX runtime">
    OpenClaw only starts the channel after explicit <code>channels.openclaw-simplex.connection</code> config exists. The plugin then starts the official SimpleX Node runtime.
  </Step>
  ...
  <Step title="Replies go back through SimpleX">
    Message actions, media sends, polls, edits, deletes, reactions, and group operations are executed through the official SimpleX runtime APIs.
  </Step>
</Steps>

<Note>
  OpenClaw does not create a hosted bot identity for this channel and does not currently expose plugin-defined invite buttons on the external plugin channel card.
</Note>
...
<Columns cols={2}>
  ...
  <Card title="Operational helpers" icon="bolt">
    Directory lookups, target resolution, heartbeat readiness, and gateway methods for runtime diagnostics, invite/address automation, contact requests, group links, and link onboarding.
  </Card>
</Columns>
