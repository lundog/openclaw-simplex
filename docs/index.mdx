...
<Columns cols={2}>
  ...
  <Card title="Runtime Setup" icon="settings" href="/guide/runtime-setup" cta="Run the runtime">
    Run <code>simplex-chat</code>, wire OpenClaw to its WebSocket endpoint, and install a supervised local service with the plugin CLI.
  </Card>
</Columns>
...
<Columns cols={2}>
  <Card title="Production Sidecar" icon="lock-keyhole" href="/guide/production-sidecar" cta="Harden deployment">
    Keep the WebSocket private, use sidecar/container patterns safely, and understand the MIT external-runtime boundary.
  </Card>
  <Card title="Configuration" icon="sliders-horizontal" href="/reference/config" cta="View fields">
    See every channel field, including <code>allowUnsafeRemoteWs</code>, media limits, and multi-account settings.
  </Card>
</Columns>
