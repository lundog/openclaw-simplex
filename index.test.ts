...
function setupRegistration(
  ...
  const api: OpenClawPluginApi = {
    ...
    emitAgentEvent: () => ({ emitted: false as const, reason: "mock" }),
    setRunContext: () => false,
    getRunContext: () => undefined,
    clearRunContext: () => {},
    registerSessionSchedulerJob: () => undefined,
    registerSessionAction: () => {},
    sendSessionAttachment: async () => ({ ok: false as const, error: "mock" }),
  ...
  it("registers gateway methods only in full mode", () => {
    ...
    expect(full.methodScopes).toEqual(
      new Map([
        ...
        ["simplex.runtime.users", "operator.read"],
        ["simplex.runtime.activeUser", "operator.read"],
        ["simplex.verification.show", "operator.read"],
        ["simplex.verification.check", "operator.admin"],
        ...
        ["simplex.groups.member.block", "operator.admin"],
        ["simplex.groups.member.deleteMessages", "operator.admin"],
        ["simplex.files.receive", "operator.admin"],
        ["simplex.files.cancel", "operator.admin"],
